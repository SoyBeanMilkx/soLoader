...
bool SoinfoManager::UpdateSoinfo(soinfo* si, MemoryManager* mm, ElfReader* reader) {
    ...
    LOGD("Updated soinfo: base=0x%lx, size=0x%zx, bias=0x%lx, entry=0x%lx, phdr=%p",
         si->base, si->size, si->load_bias, si->entry, si->phdr);
...
bool SoinfoManager::ParseDynamic(soinfo* si) {
    ...
    for (const ElfW(Phdr)* phdr = si->phdr; phdr < phdr_limit; ++phdr) {
        if (phdr->p_type == PT_DYNAMIC) {
            ...
            found_dynamic = true;
            LOGD("Found PT_DYNAMIC at vaddr=0x%lx, memsz=0x%lx", phdr->p_vaddr, phdr->p_memsz);
            break;
        }
    }
    ...
    for (ElfW(Dyn)* d = si->dynamic; d->d_tag != DT_NULL && dyn_count < si->dynamic_count; ++d, ++dyn_count) {
        ...
        LOGD("Processing dynamic entry %zu: tag=0x%lx, val/ptr=0x%lx",
             dyn_count, (unsigned long)d->d_tag, (unsigned long)d->d_un.d_val);

        switch (d->d_tag) {
            case DT_SYMTAB:
                si->symtab = reinterpret_cast<ElfW(Sym)*>(si->load_bias + d->d_un.d_ptr);
                LOGD("DT_SYMTAB: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->symtab);
                break;

            case DT_STRTAB:
                si->strtab = reinterpret_cast<const char*>(si->load_bias + d->d_un.d_ptr);
                LOGD("DT_STRTAB: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->strtab);
                break;
            ...
            case DT_HASH: {
                ...
                LOGD("DT_HASH: raw_ptr=0x%lx, nbucket=%zu, nchain=%zu",
                     (unsigned long)d->d_un.d_ptr, si->nbucket, si->nchain);
                break;
            }
            ...
            case DT_GNU_HASH: {
                ...
                LOGD("DT_GNU_HASH: raw_ptr=0x%lx, nbucket=%zu, symbias=%u",
                     (unsigned long)d->d_un.d_ptr, si->gnu_nbucket, symbias);
                break;
            }

            case DT_JMPREL:
                si->plt_rela = reinterpret_cast<ElfW(Rela)*>(si->load_bias + d->d_un.d_ptr);
                LOGD("DT_JMPREL: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->plt_rela);
                break;
            ...
            case DT_RELA:
                ...
                LOGD("DT_RELA: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->rela);
                break;
            ...
            case DT_INIT:
                ...
                LOGD("DT_INIT: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->init_func);
                break;

            case DT_INIT_ARRAY:
                si->init_array = reinterpret_cast<void (**)()>(si->load_bias + d->d_un.d_ptr);
                LOGD("DT_INIT_ARRAY: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->init_array);
                break;
            ...
            case DT_FINI:
                LOGD("DT_FINI: 0x%lx", (unsigned long)d->d_un.d_ptr);
                break;

            case DT_FINI_ARRAY:
                si->fini_array = reinterpret_cast<void (**)()>(si->load_bias + d->d_un.d_ptr);
                LOGD("DT_FINI_ARRAY: raw_ptr=0x%lx, final_addr=%p",
                     (unsigned long)d->d_un.d_ptr, si->fini_array);
                break;
            ...
            case DT_FLAGS_1:
                LOGD("DT_FLAGS_1: 0x%lx", (unsigned long)d->d_un.d_val);
                break;
            ...
            default:
                LOGD("Unknown dynamic tag: 0x%lx, value=0x%lx",
