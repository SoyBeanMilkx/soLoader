    ...
    if (loaded_si_->symtab != nullptr) {
        if (loaded_si_->gnu_bucket != nullptr) {
            ...
            if (sym != nullptr && sym->st_shndx != SHN_UNDEF) {
                ...
                LOGD("Found symbol %s via GNU hash: st_value=0x%lx, load_bias=0x%lx, final_addr=0x%lx",
                     name, sym->st_value, loaded_si_->load_bias, addr);

                if (addr >= loaded_si_->base && addr < loaded_si_->base + loaded_si_->size) {
                    return reinterpret_cast<void*>(addr);
                } else {
                    LOGE("Symbol %s address 0x%lx out of range [0x%lx, 0x%lx)",
                         name, addr, loaded_si_->base, loaded_si_->base + loaded_si_->size);
                }
            } else {
        ...
        if (loaded_si_->bucket != nullptr) {
            ...
            if (sym != nullptr && sym->st_shndx != SHN_UNDEF) {
                ElfW(Addr) addr = sym->st_value + loaded_si_->load_bias;
                LOGD("Found symbol %s via ELF hash: st_value=0x%lx, load_bias=0x%lx, final_addr=0x%lx",
                     name, sym->st_value, loaded_si_->load_bias, addr);
                ...
                for (size_t i = 0; i < sym_count; ++i) {
                    ...
                    if (sym->st_name != 0 && sym->st_shndx != SHN_UNDEF) {
                        ...
                        if (strcmp(sym_name, name) == 0) {
                            ElfW(Addr) addr = sym->st_value + loaded_si_->load_bias;
                            LOGD("Found symbol %s via linear search: st_value=0x%lx, load_bias=0x%lx, final_addr=0x%lx",
                                 name, sym->st_value, loaded_si_->load_bias, addr);
