...
bool Relocator::LinkImage(soinfo* si) {
    ...
    if (si->init_func != nullptr) {
        ...
        try {
            ...
            LOGE("Exception in init function");
            return false;
        }
    }
        ...
        for (size_t i = 0; i < si->init_array_count; ++i) {
            ...
            if (func != nullptr) {
                ...
                try {
                    ...
                    LOGE("Exception in init_array[%zu]", i);
                }
            }
        }
    }
...
bool Relocator::ProcessRelaRelocation(soinfo* si, const ElfW(Rela)* rela) {
    ...
    LOGD("Processing relocation: offset=0x%lx, type=%d, sym=%d, addend=0x%lx",
         rela->r_offset, type, sym, rela->r_addend);
    ...
    if (sym != 0) {
        ...
        if (s->st_shndx != SHN_UNDEF) {
            ...
            LOGD("Local symbol: addr=0x%lx", sym_addr);
        } else if (sym_name) {
    ...
    switch (type) {
        ...
        case R_AARCH64_ABS64:
            LOGD("R_AARCH64_ABS64: writing 0x%lx to 0x%lx", sym_addr + rela->r_addend, reloc);
            *reinterpret_cast<ElfW(Addr)*>(reloc) = sym_addr + rela->r_addend;
            break;

        case R_AARCH64_GLOB_DAT:
            LOGD("R_AARCH64_GLOB_DAT: writing 0x%lx to 0x%lx", sym_addr + rela->r_addend, reloc);
            *reinterpret_cast<ElfW(Addr)*>(reloc) = sym_addr + rela->r_addend;
            break;

        case R_AARCH64_JUMP_SLOT:
            LOGD("R_AARCH64_JUMP_SLOT: writing 0x%lx to 0x%lx", sym_addr + rela->r_addend, reloc);
            *reinterpret_cast<ElfW(Addr)*>(reloc) = sym_addr + rela->r_addend;
            break;

        case R_AARCH64_RELATIVE:
            LOGD("R_AARCH64_RELATIVE: writing 0x%lx to 0x%lx", si->load_bias + rela->r_addend, reloc);
        ...
        case R_AARCH64_IRELATIVE:
        {
            ...
            LOGD("R_AARCH64_IRELATIVE: resolver at 0x%lx", resolver);
            ...
            try {
                ...
                *reinterpret_cast<ElfW(Addr)*>(reloc) = resolved;
                LOGD("R_AARCH64_IRELATIVE: resolved to 0x%lx", resolved);
            } catch (...) {
                LOGE("Exception in IRELATIVE resolver");
                return false;
            }
        }
            break;
    ...
    if (si->symtab != nullptr) {
        if (si->gnu_bucket != nullptr) {
            ...
            if (sym != nullptr && sym->st_shndx != SHN_UNDEF) {
                ...
                LOGD("Found symbol %s in current SO at 0x%lx", name, addr);
                return addr;
            }
        }
