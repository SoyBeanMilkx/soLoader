...
bool MemoryManager::ReserveAddressSpace(const ElfW(Phdr)* phdr_table, size_t phdr_num) {
    ...
    LOGD("Load size: 0x%zx, min_vaddr: 0x%lx", load_size_, min_vaddr);
    ...
    LOGD("Reserved address space at %p, bias: 0x%lx", start, load_bias_);
...
bool MemoryManager::LoadSegments(const ElfW(Phdr)* phdr_table, size_t phdr_num,
    ...
    for (size_t i = 0; i < phdr_num; ++i) {
        ...
        LOGD("Processing LOAD segment %zu: vaddr=0x%lx, memsz=0x%lx, filesz=0x%lx, offset=0x%lx",
        ...
        if (file_end > file_size) {
            LOGE("Invalid file size: file_end=0x%lx > file_size=0x%zx", file_end, file_size);
            return false;
        }
        ...
        if (phdr->p_filesz > 0) {
            ...
            LOGD("Copying segment %zu: src=%p (offset=0x%lx), dst=%p, size=0x%lx",
                 i, src, phdr->p_offset, dst, phdr->p_filesz);
        ...
        if (phdr->p_memsz > phdr->p_filesz) {
            ...
            LOGD("Zeroing BSS: start=0x%lx, size=0x%zx", bss_start, bss_size);
...
bool MemoryManager::ProtectSegments(const ElfW(Phdr)* phdr_table, size_t phdr_num) {
    for (size_t i = 0; i < phdr_num; ++i) {
        ...
        LOGD("Protected segment %zu: 0x%lx-0x%lx, prot: %d",
             i, seg_page_start, seg_page_end, prot);
    }
